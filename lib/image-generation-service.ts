...
async function generateImageDirect(params: {
  ...
  if (hasReference) {
    ...
    for (let index = 0; index < referenceImageDataUrls.length; index++) {
      const converted = dataUrlToBlob(referenceImageDataUrls[index]);
  ...
  try {
    ...
    if (contentType.includes("text/plain")) {
      ...
      if (data.error || !data.b64) {
        throw new Error(redactImageGenerationError(data.error || `生图请求失败 ${data.httpStatus ?? res.status}`));
      }
    } else {
      // 非流式回退(旧服务端等)
      ...
      if (!res.ok || data.error || !data.b64) {
        throw new Error(redactImageGenerationError(data.error || `生图请求失败 ${res.status}`));
      }
    }
...
export async function generateImageFromConfiguredApi(params: {
  ...
  if (settings.provider === "novelai") {
    ...
    return {
      ...
      userReferenceImageMessage: params.appUserReferenceImage?.dataUrl ? "NovelAI 不支持 App 用户参考图，已使用提示词生成。" : undefined,
  ...
  const rawCharRefDataUrl = params.useReferenceImage && reference?.assetId
    ? await getChatImageFromIndexedDB(reference.assetId)
    : null;
  throwIfAborted(params.signal);
  const characterRefDataUrl = rawCharRefDataUrl
    ? await normalizeReferenceImageForEdit(rawCharRefDataUrl)
    : null;
  throwIfAborted(params.signal);

  // 用户参考图
  const userReferenceRequested = Boolean(params.appUserReferenceImage?.dataUrl);
  let userRefDataUrl: string | null = null;
  let userReferenceImageStatus: UserReferenceImageStatus = userReferenceRequested ? "fallback_prompt" : "not_requested";
  ...
  if (userReferenceRequested) {
    ...
    if (policy.canAttemptImageInput) {
      userRefDataUrl = await normalizeReferenceImageForEdit(params.appUserReferenceImage!.dataUrl);
      throwIfAborted(params.signal);
    } else {
  ...
  if (userRefDataUrl) allRefs.push(userRefDataUrl);
  if (characterRefDataUrl) allRefs.push(characterRefDataUrl);

  const callGenerate = (refs: string[]) => {
    const generateParams = { settings, prompt, referenceImageDataUrls: refs, signal: params.signal };
    return settings.requestMode === "direct"
      ? generateImageDirect(generateParams)
      : generateImageViaServerOrProxy(generateParams);
  };

  let data: ImageGenerationApiResponse;
  let usedCharacterRef = Boolean(characterRefDataUrl);
  let usedUserRef = Boolean(userRefDataUrl);

  try {
    data = await callGenerate(allRefs);
  } catch (error) {
    // 用户图请求了且在数组中，且错误是「edits/multipart 不被支持」类→降级为只带角色图重试
    if (userRefDataUrl && isReferenceInputUnsupportedError(error)) {
      usedUserRef = false;
      userReferenceImageStatus = "fallback_prompt";
      userReferenceImageMessage = "当前服务商未接受 App 用户参考图，已使用提示词生成。";
      const fallbackRefs = characterRefDataUrl ? [characterRefDataUrl] : [];
      data = await callGenerate(fallbackRefs);
      usedCharacterRef = Boolean(characterRefDataUrl);
    } else {
      throw error;
    }
  }
  ...
  if (usedUserRef) {
    userReferenceImageStatus = "used";
    userReferenceImageMessage = undefined;
  }
  ...
  return {
    ...
    mimeType,
    prompt,
    usedReferenceImage: usedCharacterRef || usedUserRef,
    usedCharacterReferenceImage: usedCharacterRef,
    usedUserReferenceImage: usedUserRef,
